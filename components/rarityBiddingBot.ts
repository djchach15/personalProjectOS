...
export async function rarityBiddingBot(collectionName:any, collectionSchema:any, collection_address:any, tokenIDStart:any, collectionCount:any, defaultOffer_var:number, maxOffer_var:number, wallet_address: any, mnemonic_phrase_var: any, os_api_key:any, provider_url:any, collectionArr:any) {
    // Setup Info
    ...
    var arrID:number = 0;
    var arrLength = collectionArr.length;

    for (let token_ID = 0; token_ID < arrLength; token_ID++) {
        // Determine highest offer of each asset
        var highestOffer = await getOffers(collectionArr[token_ID], collection_address, os_api_key);
        ...
        try {
            ...
            if (collectionSchema == "ERC1155") {
                const offer = await seaport.createBuyOrder({
                    asset: {
                        ...
                        tokenId: collectionArr[token_ID].toString(),
            ...
            console.log(collectionName + ' Bid ' + biddingAmount + ' ETH on ' + collectionArr[token_ID] + ' which had an offer of ' + highestOffer);
            console.log("");
        } catch (error:any) {
        ...
        if (token_ID == (collectionArr.length-2)) {
            token_ID = 0;
